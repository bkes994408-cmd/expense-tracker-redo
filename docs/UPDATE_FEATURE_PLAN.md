...
已落地（U14）：
- recommended 更新可設定 24 小時後稍後提醒。
- optional 更新可略過目前版本；下個版本仍會重新提醒。
- required 更新不可略過，也不提供稍後提醒。

已落地（U15）：
- Settings「檢查更新」入口會顯示更新提醒狀態 badge。
- 可在入口和彈窗中確認「未設定提醒 / 稍後提醒 / 已略過版本 / 有新版本需重新確認」。

已落地（U16）：
- recommended 的稍後提醒會依 `remindAfter` 判斷是否到期。
- 到期後 Settings 入口與彈窗會顯示「提醒到期」，提醒使用者重新檢查更新。

已落地（U17）：
- 檢查更新彈窗可清除既有提醒偏好。
- 清除後會移除本機 preference，入口與彈窗回到「未設定提醒」，使用者可重新設定稍後提醒或略過版本。
