...
U1～U5 已完成版本資訊、migration registry、本機復原點、檢查更新 UX、分類/備註規則版本化。

建議後續：
1. 接入正式 App Store / Play Store 版本查詢與商店連結。
2. 建立可匯出的 migration/recovery diagnostics。
3. 規劃「主動重新套用分類規則」批次流程，但預設不覆蓋手動分類。
4. 若要做匯率換算，先定義匯率來源、歷史口徑與缺值 fallback。
