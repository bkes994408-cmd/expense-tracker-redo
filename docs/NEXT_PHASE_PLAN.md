...
U1～U7 已完成版本資訊、migration registry、本機復原點、檢查更新 UX、分類/備註規則版本化、更新診斷匯出，以及主動重新套用分類規則。

建議後續：
1. 接入正式 App Store / Play Store 版本查詢與商店連結。
2. 若要做匯率換算，先定義匯率來源、歷史口徑與缺值 fallback。
3. 規劃更完整的同步狀態中心，整合本機資料、最後匯出、本機復原點與未來雲端同步狀態。
