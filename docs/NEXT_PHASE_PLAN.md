...
U1～U11 已完成版本資訊、migration registry、本機復原點、檢查更新 UX、分類/備註規則版本化、更新診斷匯出、主動重新套用分類規則、同步狀態中心、匯率口徑準備、商店連結設定，以及遠端更新 manifest 基礎。

建議後續：
1. 接入正式 App Store / Play Store 版本查詢；商店連結已可用 `VITE_APP_STORE_URL` / `VITE_PLAY_STORE_URL` 設定，遠端版本 manifest 已可用 `VITE_UPDATE_MANIFEST_URL` 設定。
